...
import fastml.libs.vendors.tensorflow.visualization_utils as viz_utils
from fastml.labels import labels_loader
...
class ObjectDetection():
    ...
    def _load_labels(self):
        #labels_path = open('fastml/models/labels/mscoco_labels.json')
        ...
        for box, class_id, score, anchor_indice \
            ...
            if score >= self.threshold:
                results.append({'label':self.labels.get(class_id).get('name'), 'score':np.around(score*100, 2),
                            'box':box, 'anchor_indice':anchor_indice})
        return results
...
class EfficientNetD7(ObjectDetection):
    def __init__(self, threshold=0.3):
        super().__init__("efficientdet/d7/1", threshold)
